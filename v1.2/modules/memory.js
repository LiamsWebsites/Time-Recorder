import {assets, global, sounds, settings} from './index.js';
import { exportTimePassed } from './settings.js';
...
function appendTime(game, key, inp) {
    ...
    global.times[game][key].push(Number(inp.toFixed(3)));
    global.saveLog.push([game, key, Number(inp.toFixed(3)), global.times[game][key].length]);
    if (assets.Auto.checked) {
        global[game][key].push(Number(inp.toFixed(3)));
        saveGame('myApp_'+global.nameofUser, global.balloonGame, global.bedBallGame);
        global.memLog.push([game, key, Number(inp.toFixed(3)), global[game][key].length]);
    }
}
...
export function loadUser() {
    ...
    assets.balloonAvgAmount.value = global.balloonGame[assets.balloonDropdown.value].length;
    assets.bedballAvgAmount.value = global.bedBallGame[assets.bedBallDropdown.value].length;
}

export function balloonSave() {
    if (appendTime('balloonGame', assets.balloonDropdown.value, Number(assets.balloonInput.value)) === 'error') return;
...
export function bedBallSave() {
    if (appendTime('bedBallGame', assets.bedBallDropdown.value, Number(bedballInput.value))) return;
    if (global.enter !== null) {
        if (assets.check.checked) {
            if (assets.Auto.checked) {
                if (global.lastClicked === 'memory') {
...
function timerErrorMsg(x) {
    assets.timerErrorOutput.textContent = x;
    sounds.playSound(assets.warning);
    global.currFading++;
    document.dispatchEvent(assets.errClearEvent);
}

export function saveTimerTime() {
    if (settings.exportTimePassed === 0) {
        timerErrorMsg('The timer has not started yet.');
        return;
    }
    if (assets.startStop.textContent !== 'Start') {
        timerErrorMsg('The timer must be stationary.');
        return;
    }
    const tpInSecs = settings.exportTimePassed / 1000;
    appendTime(assets.saveDir1.value, assets.saveDir2.value, tpInSecs);
}